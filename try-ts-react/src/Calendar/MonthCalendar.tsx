...
export const MonthCalendar: FC<IMonthCalendarProps> = (props) => {
    ...
    function renderDays(days: DateCellList) {
        ...
        for (let i = 0; i < ROW_COUNT; i++) {
            ...
            for (let j = 0; j < COL_COUNT; j++) {
                ...
                row[j] = <div
                    ...
                    onClick={() => selectHandler?.(cell.date)}>{dateRender ? dateRender(cell.date) : (
        ...
        return rows.map((row, index) => <div key={index} className='month-calendar-body-row'>{row}</div>);
    }
    ...
    return <div className='month-calendar'>
        <div className='month-calendar-week-list'>
            {
                weekList.map((week) => {
                    return <div key={week} className='month-calendar-week-list-item'>{CalendarLocale.week[week]}</div>
                })
            }
        </div>
