...
const renderDays = (date: Date) => {
  ...
  for (let i = daysOfPrevMonth - firstDay + 1; i <= daysOfPrevMonth; i++) {
    days.push(
      <div id={`prev_${i}`} className="empty" key={`prev_${i}`}>
        {i}
      </div>,
    );
  }
  ...
  for (let i = 1; i <= daysCount; i++) {
    if (i === date.getDate()) {
      days.push(
        <div id={`day_${i}`} className="day active" key={`day_${i}`}>
          {i}
        </div>,
      );
      continue;
    ...
    days.push(
      <div id={`day_${i}`} className="day" key={`day_${i}`}>
        {i}
      </div>,
    );
  }
  ...
  if (dayOfLastDate !== 6) {
    for (let i = 1; i < 7 - dayOfLastDate; i++) {
      days.push(
        <div id={`next_${i}`} className="empty" key={`next_${i}`}>
          {i}
        </div>,
      );
    }
