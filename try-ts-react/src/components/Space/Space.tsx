...
const Space: FC<SpaceProps> = (props) => {
  ...
  const { style, className, size = space?.size || 'small', direction = 'horizontal', align, split, wrap, children, ...otherProps } = props;
  ...
  const nodes = childNodes.map((child: any, i) => {
    ...
    return (
      <Fragment key={key}>
        ...
        {i < childNodes.length - 1 && split && <span className={`${className}-split`}>{split}</span>}
      </Fragment>
    );
  });
  ...
  return (
    <div style={{ ...otherStyles, ...style }} className={classname} {...otherProps}>
      {nodes}
    </div>
  );
};
