...
const MessageItem: FC<MessageProps> = (message) => {
    ...
    return <div className='message-item' onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>{message.content}</div>
}
    ...
    const messageWrapper = <div className='message-wrapper'>
        {
            positions.map((direction) => {
                return <TransitionGroup key={direction} className={`message-wrapper-${direction}`}>
                    {
                        messageList[direction].map((message) => {
                            return <CSSTransition key={message.id} timeout={1000} classNames='message'>
